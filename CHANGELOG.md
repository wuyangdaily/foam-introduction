...
### 新增 🆕
- 添加系统与服务器用户对比excel（plus）
- 添加tg用户自主查询
- 添加最强模型OpenAi-gpt-5.4 整理媒体库（plus）此模型免费提供给大家使用（不保证100%稳定），请大家合理使用，支持整理你们非常乱的本地文件命名
> 官方提示词：调递归读取 . 目录下的所有文件夹和文件名，按 TMDB 标准整理名称，中文返回。电影格式为 名称 (年份)，剧集格式为 名称 (年份)/Season 1/名称 (年份) - S01E01，重命名文件
...
### 新增 🆕
- 将Playback Reporting插件大数据量情况下的性能改成foam的数据库获取（解决大数据量查询无法响应的问题）
...
### 新增 🆕
- 系统配置新增「Rose 管理员密码」配置项，用于到期删除用户时自动登录 Rose 并解绑账号。
- 系统配置页面新增 Rose 管理员密码编辑弹窗，支持输入新密码和二次确认。
- 用户到期被自动删除时，如果该用户已绑定 Rose，会自动尝试同步解绑 Rose 账号。
- Rose 自动解绑失败时，会向管理员发送通知，说明用户、Rose 账号和失败原因。
- Rose 自动解绑即使失败，也不会影响用户到期删除主流程。

### 优化 🛠️
- 修复影院剧场发行商剧场筛选条件无法筛选的问题
- Rose 管理员密码在系统配置列表中不再明文展示，只显示「已设置 / 未设置」。
- 修复修改其他系统配置时，密码类配置可能被占位符覆盖的问题。
- 优化 Rose 功能启用判断，避免只配置了 Rose 地址就误触发相关逻辑。
- Rose 侧已找不到绑定时，本地会自动清理绑定状态，避免残留错误绑定。
- 优化配置弹窗底部按钮布局，让保存、取消按钮显示更稳定。